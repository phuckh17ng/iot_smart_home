...
# nyc_taxi_raw_path = Path('dataset/Kitchen_Humi/raw/Kitchen_Humi.csv')
# nyc_taxi_raw_path = Path('dataset/Kitchen_Temp/raw/Kitchen_Temp.csv')
nyc_taxi_raw_path = Path('dataset/Kitchen_Gas/raw/Kitchen_Gas.csv')
labeled_data = []
with open(str(nyc_taxi_raw_path),'r') as f:
    i = 1
    for i, line in enumerate(f):
        tokens = [float(token) for token in line.strip().split(',')[1:2]]
